...
olympics_joined <- olympics_joined %>%
  mutate(medal_binary = ifelse(!is.na(medal), 1, 0),
         medal_weighted = case_when(medal == "Gold" ~ 3,
                                    ...
                                    medal == "Bronze" ~ 1,
                                    TRUE ~ 0),
         nato_wp = case_when(country == "Belgium" | country == "Canada" | country == "Denmark" | country == "France" | country == "Iceland" | country == "Italy" | country == "Luxembourg" | country == "Netherlands" | country == "Norway" | country == "Portugal" | country == "United Kingdom" | country == "United States" | country == "Greece" | country == "Turkey" | country == "Germany" | country == "Spain" ~ "NATO",
...
q2_p1 <- participate_sex %>% 
  ...
  scale_color_manual(name = "Proportion of",
                    breaks = c("Games females compete in",
...
q2_p2 <- top_15 %>%
  ...
  geom_segment(aes(yend = noc), xend = 0, colour = "grey85", size = 0.6) +
  geom_emoji(alias = 'trophy', x = top_15$n_medal_1, y = top_15$noc, 
             size = 4, color = "#D6AF36") +
  geom_emoji(alias = 'trophy', x = top_15$n_bronze_1, y = top_15$noc, 
             size = 4, color = "#824A02") +
  geom_emoji(alias = 'trophy', x = top_15$n_b_s, y = top_15$noc, 
             size = 4, color = "#A7A7AD") +
  scale_x_continuous(breaks = seq(0, 1000, 200)) +
  annotate("text", label = "Types of medals", x = 800, y = "GBR", size = rel(2.4)) +
  annotate("rect", xmin = 700, xmax = 900, ymin = "ITA", ymax = "JPN", alpha = .1) +
  annotate("point", x = 750, y = "KOR", colour = "#D6AF36", size = 1) +
  annotate("point", x = 750, y = "NOR", colour = "#824A02", size = 1) +
  annotate("point", x = 750, y = "FRA", colour = "#A7A7AD", size = 1) +
  annotate("text", x = 800, y = "KOR", label = "Gold", size = rel(2.2), colour = "#D6AF36") +
  annotate("text", x = 800, y = "NOR", label = "Silver", size = rel(2.2), colour = "#824A02") +
  annotate("text", x = 800, y = "FRA", label = "Bronze", size = rel(2.2), colour = "#A7A7AD") +
  labs(title = "Top 15 countries in female athletes contribution in Olympic Games",
       subtitle = "From year 2000 to 2016",
       x = "Number of medals",
       y = "Countries",
       caption = "Source: data from www.sports-reference.com in 2018") +
  theme(panel.grid.major.y = element_blank()) + bg_theme
q2_p2
