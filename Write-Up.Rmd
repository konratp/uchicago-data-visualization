...
countrycode[nrow(countrycode) + 1,] <- "URS" #soviet union
countrycode[nrow(countrycode) +1,] <- "FRG" #west germany
#countrycode[nrow(countrycode) + 1,] <- c("FRG", "Europe", "West Germany", "NATO")
...
q1_df %>%
  ...
  geom_vline(xintercept = 1991,
             linetype = "dashed",
             alpha = 0.75) +
  annotate("text", label = "End of World War II", x = 1945, y = 0.9, size = rel(2), hjust = 0) +
  annotate("text", label = "Dissolution of the USSR", x = 1991, y = 0.5, size = rel(2), hjust = 1) +
