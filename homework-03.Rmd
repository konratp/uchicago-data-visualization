...
For question 2, how do you set the year for which factors are reordered? How can you make them shift based on highest population?
...
#create a racing bar chart animating the population change in the 10 countries subsetted previously
ggplot(data = pop_subset,
       mapping = aes(y = fct_reorder(country, SP.POP.TOTL, .desc = FALSE), x = SP.POP.TOTL, fill = country)) +
...
pop_subset %>%
  group_by(date) %>%  
  arrange(date, ranked_pop) %>%
  ggplot(mapping = aes(y = ranked_pop,
                       group = country, 
                       fill = as.factor(country), color = as.factor(country))) +
  geom_tile(aes(y = SP.POP.TOTL/2,
                height = SP.POP.TOTL,
